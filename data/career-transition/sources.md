...
## Surface-electrical rate → refit-facility extension (issue #220, added 2026-09-02)

This pass closes the rate → employer half of the Phase 1 surface-electrical slice by giving EM/ET/IC federal-civilian yard/facility destinations beyond the private shipbuilders, and by linking the NAVSEA transition-employer to its existing `defense_employers` seed.

- Added EM, ET, and IC edges to the two TRIDENT Refit Facilities (`trf-bangor`, `trf-kings-bay`). Both employer rows already carried primary careers-page sources from #296; these are new specialty→employer edges over that established trade record (electricians, electronics mechanics, marine electrical / interior-electrical trades supporting SSBN refit), not new source claims. The `.mil` careers pages block automated re-fetch (HTTP 403), so `SourceRetrievedOn` stays 2026-09-01, matching the last successful retrieval recorded on the employer rows; `SnapshotDate` is the 2026-09-02 curation date.
- Fit scores sit below the archetypal marine-electrician shipyards (HII, Electric Boat, NASSCO) to reflect that these are refit-facility federal-civilian trades rather than new-construction electrician lines, while still `direct`.
- Set `ExistingDefenseEmployerSlug=navsea` on the Naval Sea Systems Command transition-employer row. `navsea` is a real `DEFENSE_EMPLOYER_SEEDS` slug (`lib/defense.ts`); it is `counts_as_defense: false` with no `defense_employer_locations` rows, so the link records the shared identity without implying a private-contractor location count.
- Every other unmapped transition-employer (civilian EMS operators, commercial-cyber vendors, ONI, Draper, JHU/APL, the TRF facilities themselves as GS pipelines) has no matching `DEFENSE_EMPLOYER_SEEDS` slug and is left unmapped by design — "not yet mapped," not "no opportunities."
