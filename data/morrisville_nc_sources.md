...
- **`CityPolitics`, `2016Election`, `2016PresidentPercent`, `2024 Election`, `2024PresidentPercent`, `ElectionChange`, `rep_vote_share_change_pp`, and `dem_vote_share_change_pp` are all left blank.** The North Carolina State Board of Elections results portal (`er.ncsbe.gov`) is a JavaScript-driven query tool that could not be scraped with the tools available this session, and Wikipedia/Ballotpedia's county tables did not return usable content either. Wake County is well known qualitatively as a strong Democratic stronghold (it anchors Raleigh), but I deliberately did not substitute a statewide North Carolina proxy — North Carolina is a closely contested swing state overall while Wake County itself is not, so a statewide number would be actively misleading rather than a reasonable approximation (unlike, say, a rural county that tracks its state closely). **This is the one field group in this promotion batch that most needs a follow-up pass** — pull exact Wake County (or Morrisville-specific, if available) results directly from `wake.gov`'s election-results archive or the NCSBE portal.
