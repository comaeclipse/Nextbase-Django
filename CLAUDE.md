...
VA drive times (affordability `va_primary` annotation only; needs `VA_FACILITIES_API_KEY`; run in this order):

```bash
node --env-file=.env node_modules/tsx/dist/cli.mjs scripts/migrate-va-drive-times.ts [--dry-run]
node --env-file=.env node_modules/tsx/dist/cli.mjs scripts/sync-va-drive-times.ts [--dry-run] [--missing-only]
```
...
node --env-file=.env node_modules/tsx/dist/cli.mjs scripts/sync-defense-job-listings.ts (--employer <slug> | --all) [--apply] [--no-prune] [--force]  # re-pull board, retire stale (default dry-run)
node --env-file=.env node_modules/tsx/dist/cli.mjs scripts/purge-closed-job-listings.ts [--apply] [--days N] [--employer <slug>]  # hard-delete rows closed > N days (default 90)
```
...
- **Saved preferences** (`lib/profile.ts`, `/profile`): the visitor's state-level **dealbreakers**, persisted in the versioned `vr_profile_v1` cookie (same pattern as `lib/quiz.ts`, decoded server-side so the first paint is already personalized; a version bump discards rather than migrates). They are **hard filters, not a Fit-score factor** — `calculateBaselineScore` is untouched, and what changes is only *which* cities get ranked. The soft-weight path (`calculatePersonalizedScore` / `PersonalizedWeights`) belongs to `/quiz2` (see the Personalized Fit bullet); the two cookies are orthogonal — `vr_profile_v1` decides which cities rank, `vr_quiz2_v1` reshapes the score. `preferencesToFilterParams` maps them onto the existing shared `FilterParams` so `/profile`'s live count, `/explore`, and `/api/locations` all run the same `filterAndSort`; an untouched profile maps to all-null (a no-op). `/explore` composes them with `applyPreferenceFloor`, which is an **intersection, not a default** — the filter bar can narrow further but can never widen past a saved dealbreaker. `blockedByPreferences` explains an incompatible city on `/city/[id]` instead of hiding it. Adding a facet = one `PREFERENCE_FACETS` entry + one line in the mapper.
- **Personalized Fit** (`lib/quiz2.ts`, `/quiz2`, issues #2/#3): the visitor's weighted priorities — 0-100 sliders for the six `PersonalizedWeights` factors (VA, cost, home, safety, LGBTQ, gun rights) plus climate/lifestyle/activities/price hard filters — persisted in the versioned `vr_quiz2_v1` cookie (same server-decoded pattern as `lib/profile.ts`/`lib/quiz.ts`; a version bump discards). This is the **weighted profile**, distinct from `/profile`'s dealbreakers: it feeds `calculatePersonalizedScore` and reshapes the **Veteran Fit Score itself**, whereas `/profile` only changes which cities rank. `/city/[id]` reads the cookie server-side and, when a saved profile exists **and** the row `is_candidate`, swaps the ring/hero number, the six-factor breakdown (`calculatePersonalizedBreakdown`, each factor tagged with its `weightShare` of the total), and the copy to the personalized version; with no profile it shows the baseline five-factor score and the "build a profile" invite. `hasActiveQuiz2Profile` treats a cookie equal to the defaults as "no profile". `/quiz2` is now the primary linked flow (nav "Personalize"); the flashcard `/quiz` (`lib/quiz.ts`, hard-AND dealbreakers, no weights) still exists but is no longer linked from the city rail.
...
- **Defense job listings** (`lib/defense-jobs.ts`, `lib/defense-jobs-sectors.ts`, `/defense-jobs`): a standalone table of *individual* scraped job listings (`defense_job_listings`, one row per opening: title, pay, apply URL), imported from per-board CSVs (`data/<company>_<ats>_<date>.csv`, or `data/master_defense_jobs.csv`) by `scripts/import-defense-job-listings.ts` (upsert on `url`, re-runnable). **Where each employer's listings come from is recorded in `DEFENSE_EMPLOYER_SEEDS` (`ats_kind` + `ats_config`: board token / domain / site, `manual: true` for browser-only boards, `fetcher` for committed pull scripts) — that seed is the single registry of scrape sources (issue #313)**; company label → slug lives in `lib/defense-jobs-companies.ts`, and `lib/defense-jobs-companies.test.ts` fails if a committed listings CSV names an employer whose seed lacks a board. Distinct in granularity from `defense_employer_locations`, which holds *aggregate per-city posting counts* — the page keeps the two apart on purpose: listings render as solid, count-sized city dots; tracked-employer aggregate counts (Raytheon et al., only those with coordinates) render as **dashed "count-only" markers** the user toggles on, labeled as such. The **broad sector** filter is *derived*, not from the CSV's company-specific `Field` (which is a business unit — "X-BAT Division", "Hivemind") — `classifySector()` in `lib/defense-jobs-sectors.ts` maps title→one of nine cross-employer sectors (unit-tested); the raw `Field` is kept as a per-listing sub-team tag (both, per the user's choice). US cities are geocoded via a small hardcoded table in the importer (~25 cities); international rows stay in the list, off the US map. The four CSV companies (Shield AI, Palantir, Saronic, Vannevar Labs) plus Kratos are seeded into `DEFENSE_EMPLOYER_SEEDS` so listings link an `employer_slug` and the employer filter is unified. Not a Fit-score factor; independent of `locations_location`. **The page never ships all ~12k listings to the client** (that made a 34MB document): the server component renders only page 1 + the filter-chip facets + a city-level map aggregation + the tracked-employer counts, and `DefenseJobsExplorer` fetches everything else from two routes — `/api/defense-jobs` (filtered, paginated listings, 50/page, "Load more") and `/api/defense-jobs/map` (city `GROUP BY` rollup with a per-employer `jsonb_agg` breakdown, refetched only when filters change, not on pagination). Filtering is **server-side** via a shared parameterized WHERE builder (`buildWhere`/`parseDefenseJobFilter` in `lib/defense-jobs.ts`), so the chips/search/city-select are SQL, not client-side array filtering; LIKE wildcards in search are escaped to stay a literal substring match. The empty-filter initial reads are `unstable_cache`d (`getDefenseJobInitialListings`/`getDefenseJobInitialMap`, 300s, `defense-jobs` tag); the page stays `force-dynamic`. Selecting a city still collapses the map to that one dot (parity with the old client behavior). `getDefenseJobListings` (whole-table read) is retained but unused by the page. **Listing lifecycle (issue #313):** `last_seen_at` is bumped by every importer upsert and `closed_at` is set only by a whole-board sync when a URL is gone (the importer clears it if the URL reappears); every read filters `closed_at IS NULL`, probing `information_schema` first so the page serves before `scripts/migrate-defense-job-listings.ts` has added the columns. The importer never closes anything, and `--clear` truncates the whole shared table — never use it for a single-company refresh. **Retention (the #313 policy call, decided):** closed rows are kept for a bounded window, then hard-deleted by `scripts/purge-closed-job-listings.ts` (default `CLOSED_RETENTION_DAYS` = 90 in `lib/defense-jobs.ts`) — not kept forever. They are invisible to readers and have no consumer, so unbounded growth on high-churn boards buys nothing; the window still exceeds a normal repost gap, so a listing that returns inside it reopens the original row (preserving `created_at`). The purge is default-dry-run + `--apply` and only ever touches rows already closed longer than the window, never a live listing; run it after a `--all` refresh (the future Phase-5 cron does both).
- **Career transition** (`lib/career-transition.ts`, `/career-transition`, five tables — see SCHEMA.md): a **curated hand-built graph** mapping a military specialty → civilian roles + transition employers, sorted by `fit_score` then `directness`. It is a **seed, not full MOS coverage** (aviation-maintenance + ordnance + cyber + EW slices; `data/career-transition/sources.md`), **not a Fit-score factor**, and independent of `locations_location` except the optional `transition_employers.defense_employer_slug` → `defense_employers.slug` link. **Chat (`/chat`) stays city-only and has no career tool until Phase 4** of the [Career Transition GitHub project](https://github.com/comaeclipse/Nextbase-Django/issues/218). The integrity rule (issue #221, `resolveSpecialty`): **code owns the match; the model only narrates** — an uncovered or ambiguous specialty must ask or return empty with an explanation, **never a nearby code** (substring search wrongly resolves "navy electrician" to the aviation rate `AE`). Data PRs seed the CSVs; the DB is loaded from `master` after merge (`scripts/import-career-transition.ts`, upsert), never from a feature branch.
